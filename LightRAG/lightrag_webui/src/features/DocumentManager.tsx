...
// Parse the most recent "Chunk X of Y extracted" message from history
...
// Count completed LLMmrg (entity merge) operations from history
...
export default function DocumentManager() {
  ...
  // Per-doc rolling window of chunk completions for accurate ETA: Map<chunkTotal, samples[]>
  const chunkSamplesRef = useRef<Map<number, {n: number; t: number}[]>>(new Map())
  // Which doc's progress to show in the banner (identified by its total chunk count)
  const [selectedBannerTotal, setSelectedBannerTotal] = useState<number | null>(null)
  ...
  // Pipeline progress state (polled independently of document list)
  const [pipelineStatus, setPipelineStatus] = useState<PipelineStatusResponse | null>(null)
  const [now, setNow] = useState(Date.now())

  // Map of chunkTotal → latestCurrent built from pipeline history messages
  const chunkProgressMap = useMemo(() => {
    const map = new Map<number, number>()
    if (pipelineStatus?.busy) {
      for (const msg of pipelineStatus.history_messages ?? []) {
        const m = /Chunk (\d+) of (\d+) extracted/.exec(msg)
  ...
  useEffect(() => {
    if (!pipelineBusy) {
      ...
      chunkSamplesRef.current = new Map()
      setSelectedBannerTotal(null)
      return
    }
    let cancelled = false
    const poll = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          // Update per-doc rolling window samples for ETA
          // Scan all history to get latest chunk seen per doc (keyed by total)
          const msgs = data.history_messages ?? []
          const latestPerTotal = new Map<number, number>()
          for (const msg of msgs) {
            const m = /Chunk (\d+) of (\d+) extracted/.exec(msg)
            if (m) latestPerTotal.set(parseInt(m[2], 10), parseInt(m[1], 10))
          }
          const updatedMap = new Map(chunkSamplesRef.current)
          const t = Date.now()
          for (const [total, n] of latestPerTotal) {
            const existing = updatedMap.get(total) ?? []
            if (!existing.length || existing[existing.length - 1].n !== n) {
              updatedMap.set(total, [...existing.slice(-19), { n, t }])
            }
          }
          chunkSamplesRef.current = updatedMap
        }
      } catch { /* ignore */ }
    }
    poll()
    const iv = setInterval(poll, 2000)
    const tickIv = setInterval(() => setNow(Date.now()), 1000)
  ...
  // Docs that are "failed" in DB but actively being reprocessed — should count as Processing, not Failed
  const reprocessingCount = useMemo(() => {
    if (!pipelineStatus?.busy || !pipelineStatus.job_name) return 0
    return (filteredAndSortedDocs ?? []).filter(doc => {
      if (doc.status !== 'failed') return false
      const prefix = (doc.file_path ?? '').split(/\s+/).slice(0, 2).join(' ')
      return prefix && pipelineStatus.job_name!.toLowerCase().includes(prefix.toLowerCase())
    }).length
  }, [pipelineStatus, filteredAndSortedDocs])
  const displayProcessingCount = processingCount + reprocessingCount
  const displayFailedCount = Math.max(0, failedCount - reprocessingCount)
  ...
  return (
    <Card className="!rounded-none !overflow-hidden flex flex-col h-full min-h-0">
      ...
      <CardContent className="flex-1 flex flex-col min-h-0 overflow-auto">
        ...
        {pipelineBusy && pipelineStatus && (() => {
          const jobStart = pipelineStatus.job_start ? new Date(pipelineStatus.job_start).getTime() : null
          const elapsed = jobStart ? now - jobStart : 0

          // Build list of actively processing docs with their resolved totals
          const activeTotals = new Set(Array.from(chunkProgressMap.keys()))
          const activeDocInfos = (filteredAndSortedDocs ?? [])
            .filter(doc => {
              const isProcessing = doc.status === 'processing' &&
                (activeTotals.size === 0 || activeTotals.has(doc.chunks_count ?? -1))
              const filePrefix = (doc.file_path ?? '').split(/[\\/]/).pop()?.replace(/\.pdf$/i, '').slice(0, 20) ?? ''
              const isReprocessing = doc.status === 'failed' && !!pipelineStatus.busy && !!filePrefix &&
                !!pipelineStatus.job_name?.toLowerCase().includes(filePrefix.toLowerCase())
              return isProcessing || isReprocessing
            })
            .map(doc => {
              const rawTotal = doc.chunks_count ?? 0
              const total = rawTotal > 0
                ? rawTotal
                : Array.from(chunkProgressMap.keys()).find(t =>
                    !(filteredAndSortedDocs ?? []).some(d => d !== doc && d.chunks_count === t)
                  ) ?? 0
              const current = total > 0 ? chunkProgressMap.get(total) ?? 0 : 0
              const name = (doc.file_path ?? doc.id).split(/[\\/]/).pop()?.replace(/\.pdf$/i, '') ?? doc.id
              return { doc, total, current, name }
            })

          // Keep selected doc if still active, else auto-pick first
          const effectiveTotal = activeDocInfos.find(d => d.total === selectedBannerTotal)?.total
            ?? activeDocInfos[0]?.total ?? 0
          const sel = activeDocInfos.find(d => d.total === effectiveTotal) ?? activeDocInfos[0]

          const total = sel?.total ?? 0
          const current = sel?.current ?? 0
          const pct: number | null = total > 0 ? Math.min(99, Math.round((current / total) * 100)) : null

          // Per-doc ETA
          let remaining: string | null = null
          if (total > 0) {
            const samples = chunkSamplesRef.current.get(total) ?? []
            if (samples.length >= 2) {
              const w = samples.slice(-10)
              const msPerChunk = (w[w.length - 1].t - w[0].t) / (w[w.length - 1].n - w[0].n)
              if (msPerChunk > 0 && isFinite(msPerChunk)) {
                remaining = formatDuration(msPerChunk * (total - current))
              }
            }
          }

          const mergeCount = parseMergeProgress(pipelineStatus.history_messages ?? [])
          // Only show merge status for the selected doc if it actually completed extraction (total known)
          const isMerging = total > 0 &&
            (mergeCount > 0 || (pipelineStatus.latest_message ?? '').includes('LLMmrg'))
          const progressLabel = total > 0 && current > 0 && !isMerging
            ? `Chunk ${current.toLocaleString()} / ${total.toLocaleString()} · extracting entities`
            : isMerging
              ? `Merging knowledge graph · ${mergeCount.toLocaleString()} entities merged`
              : total === 0
                ? 'Chunking document…'
                : pipelineStatus.latest_message || 'Extracting entities…'
          const indeterminate = pct === null

          // Colour scheme switches blue→purple during merge phase
          const c = isMerging ? {
            ...
            tabInactive: 'bg-white text-purple-600 border-purple-300 hover:bg-purple-100 dark:bg-purple-900/60 dark:text-purple-300 dark:border-purple-700 dark:hover:bg-purple-800',
          } : {
            ...
            tabInactive: 'bg-white text-blue-600 border-blue-300 hover:bg-blue-100 dark:bg-blue-900/60 dark:text-blue-300 dark:border-blue-700 dark:hover:bg-blue-800',
          }

          return (
            <div className={`mb-2 rounded-md border ${c.border} ${c.bg} p-3`}>
              <style>{`
                @keyframes lr-indeterminate {
                  0%   { transform: translateX(-100%) scaleX(0.4); }
                  50%  { transform: translateX(50%)  scaleX(0.6); }
                  100% { transform: translateX(200%) scaleX(0.4); }
                }
                .lr-indeterminate { animation: lr-indeterminate 1.6s ease-in-out infinite; }
              `}</style>
              <div className="flex justify-between items-start mb-1.5 gap-2">
                {activeDocInfos.length > 1 ? (
                  <div className="flex gap-1 flex-wrap flex-1 min-w-0">
                    {activeDocInfos.map(({ total: t, name }) => (
                      <button
                        key={t}
                        onClick={() => setSelectedBannerTotal(t)}
                        title={name}
                        className={cn(
                          'text-xs px-2 py-0.5 rounded-full border transition-colors truncate max-w-[220px]',
                          effectiveTotal === t ? c.tabActive : c.tabInactive
                        )}
                      >
                  ...
                  <span className={`text-sm font-medium ${c.title} truncate flex-1 min-w-0`}>
                    {sel?.name ?? (pipelineStatus.job_name ?? 'Indexing…').replace(/\s*\[\d+\s*files?\]/i, '')}
                  </span>
                )}
                <span className={`text-xs ${c.meta} shrink-0`}>
                  {isMerging ? 'Merging…' : indeterminate ? 'Processing…' : `${pct}%`}
                  {elapsed > 0 && ` · ${formatDuration(elapsed)} elapsed`}
                  {remaining && ` · ~${remaining} left`}
                </span>
              </div>
              <div className={`w-full ${c.track} rounded-full h-2 overflow-hidden relative`}>
                {indeterminate || isMerging ? (
                  <div className={`absolute inset-0 ${c.bar} h-2 rounded-full lr-indeterminate`} />
                ) : (
                  <div
              ...
              <div className={`text-xs ${c.label} mt-1.5 truncate`}>{progressLabel}</div>
            </div>
          )
        })()}
              ...
              <div className="flex items-center gap-2">
                <div className="flex gap-1" dir={i18n.dir()}>
                  ...
                  <Button
                    ...
                    disabled={isRefreshing}
                    className={cn(
                      displayProcessingCount > 0 ? 'text-blue-600' : 'text-gray-500',
                  ...
                  >
                    {t('documentPanel.documentManager.status.processing')} ({displayProcessingCount})
                  </Button>
                  <Button
                    size="sm"
                    ...
                    <TableBody className="text-sm overflow-auto">
                      {filteredAndSortedDocs && filteredAndSortedDocs.map((doc) => (
                        <TableRow key={doc.id}>
                          ...
                          <TableCell>
                            {(() => {
                              // Unified status + progress logic
                              const activeTotals = new Set(Array.from(chunkProgressMap.keys()))
                              const filePrefix = (doc.file_path ?? '').split(/\s+/).slice(0, 2).join(' ')
                              const globalMerging = (pipelineStatus?.latest_message ?? '').includes('LLMmrg')
                              const hasNoChunks = (doc.chunks_count ?? 0) === 0
                              // A doc with no chunks is queued if activeTotals is non-empty (normal queue)
                              // OR if history was truncated and we're in merge phase (another doc finished extraction)
                              const isQueued = doc.status === 'processing' && hasNoChunks && (
                                (activeTotals.size > 0 && !activeTotals.has(doc.chunks_count ?? -1)) ||
                                (activeTotals.size === 0 && globalMerging)
                              )
                              const filePrefix2 = (doc.file_path ?? '').split(/\s+/).slice(0, 2).join(' ')
                              const isActivelyProcessing =
                                (doc.status === 'processing' && !isQueued && (activeTotals.size === 0 || activeTotals.has(doc.chunks_count ?? -1))) ||
                                (doc.status === 'failed' && !!pipelineStatus?.busy && !!filePrefix2 &&
                                  !!pipelineStatus.job_name?.toLowerCase().includes(filePrefix2.toLowerCase()))

                              // Resolve chunk progress: for docs reset to chunks=-1, find unclaimed total
                              const total = (doc.chunks_count ?? 0) > 0
                                ? doc.chunks_count!
                                : Array.from(chunkProgressMap.keys()).find(t =>
                                    !filteredAndSortedDocs.some(d => d !== doc && d.chunks_count === t)
                                  ) ?? 0
                              const current = isActivelyProcessing && total > 0 ? chunkProgressMap.get(total) ?? 0 : 0
                                ...
                                <div>
                                  <div className="group relative flex items-center overflow-visible tooltip-container">
                                    ...
                                    {(doc.error_msg || (doc.metadata && Object.keys(doc.metadata).length > 0) || doc.track_id) && (
                                      <div className="invisible group-hover:visible tooltip">
                                        {doc.track_id && <div className="mt-1">Track ID: {doc.track_id}</div>}
                                        {doc.metadata && Object.keys(doc.metadata).length > 0 && (
                                  ...
                                  {/* Inline per-doc progress bar */}
                                  {isActivelyProcessing && (() => {
                                    const docMergeCount = parseMergeProgress(pipelineStatus?.history_messages ?? [])
                                    const docIsMerging = total > 0 && current >= total * 0.99 &&
                                      (docMergeCount > 0 || (pipelineStatus?.latest_message ?? '').includes('LLMmrg'))
                                    if (total > 0 && !docIsMerging) {
                                      return (
                                    ...
                                    if (docIsMerging) {
                                      return (
                                        <div className="mt-1.5 min-w-[120px]">
                                          <style>{`.lr-mrg{animation:lr-indeterminate 1.6s ease-in-out infinite}`}</style>
                                          <div className="h-1.5 bg-purple-100 dark:bg-purple-900 rounded-full overflow-hidden relative">
                                            <div className="absolute inset-0 bg-purple-500 h-1.5 rounded-full lr-mrg" />
                                          </div>
                                          <div className="text-xs text-purple-600 dark:text-purple-400 mt-0.5 whitespace-nowrap">
                                            Merging · {docMergeCount.toLocaleString()} entities
                                          </div>
                                        </div>
                                      )
                                    }
