...
class PGVectorStorage(BaseVectorStorage):
    ...
    async def setup_table(
        ...
        if (new_table_exists and not legacy_exists) or (
            ...
            # Use LIMIT 1 instead of COUNT(*) to avoid full table scan on large
            # vector tables (COUNT(*) can time out on Supabase free tier with
            # 10k+ rows of 1536-dim embeddings due to heap scan cost).
            workspace_count_query = (
                f"SELECT 1 as count FROM {table_name} WHERE workspace = $1 LIMIT 1"
            )
            workspace_count_result = await db.query(workspace_count_query, [workspace])
            workspace_count = 1 if workspace_count_result else 0
